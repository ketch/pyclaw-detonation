...
def characteristic(current_data):
    """Computes the pressure from the conserved quantities"""
    q = current_data.q
    u = q[1,:]/q[0,:]
    c = np.sqrt(gamma*pressure(current_data)/q[0,:])    
    return u + c
...
#    Figure for Y
    plotfigure = plotdata.new_plotfigure(name='char', figno=3)

    # Set up for axes in this figure:
    plotaxes = plotfigure.new_plotaxes()
    plotaxes.title = 'charecteristic'

    # Set up for item on these axes:
    plotitem = plotaxes.new_plotitem(plot_type='1d')
    plotitem.plot_var = characteristic
    plotitem.plotstyle = 'o-'
    plotitem.color = 'b'
