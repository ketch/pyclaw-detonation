! NOTE from LMF: Solver obtained from Christiane Helzel. Not sure it still works.
...
*
       ...
       do 200 i = 2-mbc, mx+mbc
         ...
         zl = qr(i-1,4)
         zr = ql(i,4)
         pl = (gam-1.)*(qr(i-1,3)-0.5d0*qr(i-1,2)*ul
     &                  -qchem*zl*rhol)
...
*
     ...
     &                 +qchem*(rhomr*ql(i,4)-rhoml*qr(i-1,4))
...
c
         ...
         p1 = (gam-1.d0)*(en1 - 0.5d0*rhou1**2/rho1-qchem*rho1*z1)
         c1 = dsqrt(gam*p1/rho1)
         s1 = rhou1/rho1 - c1  !# u-c to right of 1-wave
         if (s0.lt.0.d0 .and. s1.gt.0.d0) then
c             write(6,*), "transonic rarefaction in the 1-wave"
...
c
         ...
         p2 = (gam-1.d0)*(en2-0.5d0*rhou2**2/rho2-qchem*rho2*z2)
